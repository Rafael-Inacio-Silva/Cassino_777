...
// cloneNode garante instância independente a cada play
...
function playChipSfx(which) {
  const src = which === 1 ? 'src/sfx/sfx/chipPut.mp3' : 'src/sfx/sfx/chipPut2.mp3';
  const a = new Audio(src);
  a.volume = parseFloat(document.querySelector('#soundFx')?.value || 0.5);
  a.play().catch(() => {});
}
...
function changeLang(lang) {
  ...
  setTimeout(() => {
    ...
    if (hasHistory) document.querySelector('#statisticBtn').classList.remove('disabledCircleBtns');
  }, 500);
  ...
  // Traduz botão Lobby de acordo com o idioma
  const lobbyEl = document.getElementById('backToLobby');
  if (lobbyEl) {
    const lobbyTexts = { en: '← Lobby', ru: '← Лобби', fr: '← Accueil' };
    const key = lang === enLang ? 'en' : lang === ruLang ? 'ru' : 'fr';
    lobbyEl.textContent = lobbyTexts[key] || '← Lobby';
  }
}
...
function endroll() {
  ...
  let rollingInterval = setInterval(function () {
    ...
    rollSound.pause();
    // Sempre re-renderiza o saldo — `money` é o valor liquidado pelo servidor
    moneyInfo.innerHTML = currentLang[0] + money.toLocaleString('en-US') + ' $';
    totalWin.innerHTML =
    ...
    setTimeout(() => {
      ...
      (async () => {
        ...
        betSize = 0;
        totalBet.innerHTML = `${currentLang[1]}0 $`;
        moneyInfo.innerHTML = currentLang[0] + money.toLocaleString('en-US') + ' $';
        spinSessionClear();
        saveLastBall(random, haben);
        saveResultHistory(random, lastBetSize, moneyTotal);
      })();
    }, 3000);
...
function restartGame() {
  ...
  money = preMoney = 0;
  loadRealBalance(); // saldo real vem do servidor — sem reset de demo
...
async function start() {
  ...
  if (betSize <= 0) {
    showSpinError(currentLang === ruLang ? 'Сделайте ставку' : currentLang === frLang ? 'Placez une mise pour lancer' : 'Faça uma aposta para girar');
    return;
  }

  // Trava a mesa e envia as apostas — sorteio e liquidação acontecem no servidor
  ...
  let spinData;
  try {
    spinData = await requestSpin(serializeBets());
  } catch (err) {
    // Falha na rodada — nada foi debitado; destrava a mesa com as apostas intactas
  ...
  rollSound.currentTime = 0;
  rollSound.play();
  setTimeout(() => { if (betStart) { endRollSound.currentTime = 0; endRollSound.play(); } }, SPIN_TOTAL_MS - 400);
  random = spinData.result; // número sorteado pelo servidor (RNG criptográfico)
  ...
  // O cálculo local acima serve só para os efeitos visuais (winSum);
  // o saldo final é o liquidado pelo servidor.
  pendingMoney = parseFloat(spinData.balance);
  money = balanceBeforeSpin;
  moneyInfo.innerHTML = `${currentLang[0]}${money.toLocaleString('en-US')} $`;
  spinSessionSave({ random, haben, money, pendingMoney, lastBetSize, preMoneyAtStart: preMoney, startTime: Date.now(), startAngle: i });
...
const LAST_BALL_KEY = 'roulette_last_ball';
const HISTORY_KEY   = 'roulette_results';

function saveLastBall(num, ballAngle) {
  try { localStorage.setItem(LAST_BALL_KEY, JSON.stringify({ num, ballAngle })); } catch (e) {}
}

function restoreLastBall() {
  try {
    const d = JSON.parse(localStorage.getItem(LAST_BALL_KEY));
    if (!d) return;
    ball.style.display = 'block';
    ballSpin.style.transform = `rotate(${d.ballAngle}deg)`;
    ball.style.animationDelay = '-800ms';
    ball.style.animationPlayState = 'running';
  } catch (e) {}
}

function saveResultHistory(num, betAmt, profit) {
  try {
    const arr = JSON.parse(localStorage.getItem(HISTORY_KEY) || '[]');
    const color = num === 0 ? 'green' : infoConstants.sectionNumbers.section2.includes(num) ? 'red' : 'black';
    arr.push({ num, color, betAmt, profit, ts: Date.now() });
...
function renderResultStrip() {
  ...
  try {
    const arr = JSON.parse(localStorage.getItem(HISTORY_KEY) || '[]');
    if (arr.length === 0) { strip.classList.remove('has-results'); strip.innerHTML = ''; return; }
    const recent = arr.slice(-50).reverse();
    const balls = recent.map((r, idx) =>
      `<span class="rs-ball rs-${r.color}${idx === 0 ? ' rs-first' : ''}" title="Rodada ${r.num}">${r.num}</span>`
    ).join('');
    strip.classList.add('has-results');
    strip.innerHTML = `<div class="rs-label">&#9679; Últimos sorteios</div><div class="rs-grid">${balls}</div><div class="rs-divider"></div>`;
  } catch (e) {}
}
...
function clearResultHistory() {
  ...
  if (historyMenu) historyMenu.innerHTML = '';
}

function spinSessionSave(data) {
  try { localStorage.setItem(SPIN_KEY, JSON.stringify(data)); } catch (e) {}
}
function spinSessionGet() {
  try { return JSON.parse(localStorage.getItem(SPIN_KEY)); } catch (e) { return null; }
}
function spinSessionClear() {
...
async function loadRealBalance() {
  ...
  if (!token) return;
  // Skip if there's an active spin session — tryResumeActiveSpin manages state
  if (spinSessionGet()) return;
  try {
    const res = await fetch('/api/wallet/balance', { headers: { Authorization: `Bearer ${token}` } });
    if (res.ok) {
      const data = await res.json();
      money = preMoney = parseFloat(data.balance);
      if (moneyInfo) moneyInfo.innerHTML = `${currentLang[0]}${money.toLocaleString('en-US')} $`;
      checkMoney();
    }
  } catch (e) {}
}

// Serializa as apostas da mesa no formato aceito por POST /api/roulette/spin
function serializeBets() {
  const pick = (obj) => {
    const out = {};
    for (const k in obj) if ((obj[k] || 0) > 0) out[k] = obj[k];
    return out;
  };
  return {
    number:  pick(numberBets),
    split:   pick(splitBetsX),
    splitY:  pick(splitBetsY),
    corner:  pick(cornerBets),
    street:  pick(streetBets),
    sixain:  pick(sixainBets),
    section: pick(sectionBets),
  };
}

// Envia a rodada ao servidor — ele valida, debita, sorteia, paga e registra
async function requestSpin(apiBets) {
  const token = localStorage.getItem('token');
  ...
  if (!res.ok) throw new Error(data.message || 'Erro ao processar a rodada.');
  return data;
}

function showSpinError(msg) {
  let el = document.querySelector('#spinError');
  if (!el) {
    el = document.createElement('div');
    el.id = 'spinError';
    Object.assign(el.style, {
      position: 'fixed', top: '4vh', left: '50%', transform: 'translateX(-50%)',
      background: '#7a1212', color: '#fff', padding: '1.4vh 2.6vh',
      borderRadius: '1vh', zIndex: '99999', fontSize: '2.2vh',
      boxShadow: '0 0 1.5vh #000', pointerEvents: 'none', textAlign: 'center',
    });
...
function tryResumeActiveSpin() {
  const saved = spinSessionGet();
  if (!saved || !saved.startTime) return false;
  ...
  random = saved.random;
  money = saved.money;
  preMoney = saved.preMoneyAtStart !== undefined ? saved.preMoneyAtStart : money;
  if (saved.lastBetSize) {
    lastBetSize = saved.lastBetSize;
    betSize = saved.lastBetSize;
  }
  pendingMoney = saved.pendingMoney ?? saved.money;
  if (moneyInfo) moneyInfo.innerHTML = `${currentLang[0]}${money.toLocaleString('en-US')} $`;
  if (totalBet && lastBetSize) totalBet.innerHTML = `${currentLang[1]}${lastBetSize.toLocaleString('en-US')} $`;

  betStart = true;
  i = (saved.startAngle + Math.floor(elapsed / 6) * 2) % 360;
  ball.style.display = 'block';
  ballSpin.style.transform = `rotate(${i}deg)`;

  [btns.doubleBet, btns.cancelLastBtn, btns.cancelAllbtn, btns.repeatBetBtn]
    .forEach((el) => el.classList.add('disabledCircleBtns'));
  document.querySelector('#betSection').style.pointerEvents = 'none';
  document.querySelector('#chipsSelector').style.filter = 'grayscale(1)';
  document.querySelector('#chipsSelectorDisabled').style.display = 'block';
  btns.betCompleteBtn.style.pointerEvents = 'none';
  ...
  const remaining = SPIN_TOTAL_MS - elapsed;
  if (remaining > 0) {
    rollSound.currentTime = Math.min(elapsed / 1000, rollSound.duration || 99);
    rollSound.play().catch(() => {});
    setIntSpin();
    endrollTimer = setTimeout(() => { if (betStart) endroll(); }, remaining);
    const soundDelay = remaining - 400;
    if (soundDelay > 0) setTimeout(() => { if (betStart) { endRollSound.currentTime = 0; endRollSound.play(); } }, soundDelay);
  } else {
    setIntSpin();
    setTimeout(() => { if (betStart) endroll(); }, 50);
  }
  return true;
}
...
window.onload = function () {
  preloadImages(imageManifest, (loaded, total, errored) => {
    ...
    .then(() => {
      ...
      rotate();
      renderResultStrip();
      if (!tryResumeActiveSpin()) restoreLastBall();
    })
    .catch((e) => {
...
if (clearHistoryBtn) clearHistoryBtn.addEventListener('click', clearResultHistory);
